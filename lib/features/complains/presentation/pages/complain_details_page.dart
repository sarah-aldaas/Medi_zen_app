...
class _ComplainDetailsPageState extends State<ComplainDetailsPage> {
  ...
  void _loadInitialData() {
    context.read<ComplainCubit>().getComplainDetails(context: context, complainId: widget.complainId);
    _loadResponses();
  }

  void _loadResponses() {
    context.read<ComplainCubit>().getComplainResponses(complainId: widget.complainId, context: context);
  }
   ...
   isClose = complain.status?.code == 'complaint_closed';
    ...
    return Column(
      children: [
        // Status indicator
        GestureDetector(
          onTap:complain.status?.code== "complaint_closed"? null:_confirmCloseComplain,
          child: Container(
            padding: const EdgeInsets.symmetric(vertical: 8),
            color: _getStatusColor(complain.status?.code).withOpacity(isDarkMode ? 0.3 : 0.2),
            child: Center(
              child: Text(
                complain.status?.display ?? 'No Status',
                style: TextStyle(
          ...
          child: BlocBuilder<ComplainCubit, ComplainState>(
            ...
            builder: (context, state) {
              if (state is ComplainResponsesSuccess) {
                // Combine and sort all messages (responses + original complaint)
                final allMessages = _combineAndSortMessages(complain, state.responses);
        ...
        // Only show attachments and input area if complaint is not closed
        if (!isClose) ...[
          if (_attachments.isNotEmpty) _buildAttachmentsPreview(isDarkMode),
          _buildInputArea(isDarkMode),
        ] else _buildClosedComplaintMessage(isDarkMode),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Center(
        child: Padding(
          ...
          child: Text(
            'This complaint is closed. No further responses can be added.',
            style: TextStyle(
  ...
  List<_MessageItem> _combineAndSortMessages(
      ComplainModel complain, List<ComplainResponseModel> responses) {
    // Convert all messages to a unified format
    final allMessages = <_MessageItem>[
      ...
      ...responses.map(
          ...
          senderName: r.sender?.fName ?? 'Staff',
