...
abstract class AuthRemoteDataSource {
  Future<Resource<AuthResponseModel>> signup({required RegisterRequestModel registerRequestModel});

  Future<Resource<AuthResponseModel>> verifyOtp({required String email, required String otp});

  Future<Resource<AuthResponseModel>> resendOtp({required String email});
...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  @override
  Future<Resource<AuthResponseModel>> signup({required RegisterRequestModel registerRequestModel}) async {
    final response = await networkClient.invoke(EndPointsAuth.register, RequestType.post, body: registerRequestModel.toJson());
    return ResponseHandler<AuthResponseModel>(response).processResponse(fromJson: (json) => AuthResponseModel.fromJson(json));
  }

  @override
  Future<Resource<AuthResponseModel>> verifyOtp({required String email, required String otp}) async {
    final response = await networkClient.invoke(EndPointsAuth.verifyOtpRegister, RequestType.post, body: {'email': email, 'otp': otp});
    return ResponseHandler<AuthResponseModel>(response).processResponse(fromJson: (json) => AuthResponseModel.fromJson(json));
  }

  @override
  Future<Resource<AuthResponseModel>> resendOtp({required String email}) async {
    final response = await networkClient.invoke(EndPointsAuth.resendOtp, RequestType.post, body: {'email': email});
    return ResponseHandler<AuthResponseModel>(response).processResponse(fromJson: (json) => AuthResponseModel.fromJson(json));
  }
