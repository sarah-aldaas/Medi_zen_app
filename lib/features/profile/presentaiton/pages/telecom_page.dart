...
class _TelecomPageState extends State<TelecomPage> {
  ...
  final CodeModel _allTab = CodeModel(id: 'all', display: 'All', code: '', description: '', codeTypeId: '');

  @override
  void initState() {
    super.initState();
    // Trigger fetching of telecom types and use codes
    context.read<CodeTypesCubit>().getTelecomTypeCodes(context: context).then((types) {
      if (mounted) {
        setState(() {
    ...
    context.read<CodeTypesCubit>().getTelecomUseCodes(context: context).then((uses) {
      if (mounted) {
        setState(() {
          _telecomUses = uses;
        });
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            ...
            style: theme.appBarTheme.titleTextStyle?.copyWith(fontWeight: FontWeight.bold) ?? TextStyle(color: theme.primaryColor, fontWeight: FontWeight.bold),
          ),
          ...
          bottom: PreferredSize(
            ...
            child:
                _telecomTypes.isEmpty
                    ? Center(child: LinearProgressIndicator(color: theme.primaryColor))
                              ...
                              return Padding(
                                ...
                                child: ChoiceChip(
                                  ...
                                  labelStyle: theme.chipTheme.labelStyle?.copyWith(
                                    color: _selectedTab == type ? theme.colorScheme.onPrimary : theme.textTheme.bodyMedium?.color,
                                  ),
                                ),
                              );
                            }).toList(),
