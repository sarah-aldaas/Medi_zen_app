...
class CodeTypesCubit extends Cubit<CodeTypesState> {
  ...
  Future<List<CodeModel>> getAppointmentTypeCodes() async {
    ...
    if (!currentCodes.any((code) => code.codeTypeModel!.id == appointmentTypeCodeType.id)) {
      await fetchCodes(codeTypeId: appointmentTypeCodeType.id, codeTypes: codeTypes);
    }

    final updatedState = state;
    if (updatedState is CodeTypesSuccess) {
      return updatedState.codes?.where((code) => code.codeTypeModel!.id == appointmentTypeCodeType.id).toList() ?? [];
    }
    return [];
  }
  ...
  Future<List<CodeModel>> getAddressTypeCodes() async {
    ...
    if (!currentCodes.any((code) => code.codeTypeModel?.id == addressTypeCodeType.id)) {
      await fetchCodes(codeTypeId: addressTypeCodeType.id, codeTypes: codeTypes);
    }

    final updatedState = state;
    if (updatedState is CodeTypesSuccess) {
      return updatedState.codes?.where((code) => code.codeTypeModel?.id == addressTypeCodeType.id).toList() ?? [];
    }
    return [];
  }
  ...
  Future<List<CodeModel>> getAddressUseCodes() async {
    ...
    if (!currentCodes.any((code) => code.codeTypeModel?.id == addressUseCodeType.id)) {
      await fetchCodes(codeTypeId: addressUseCodeType.id, codeTypes: codeTypes);
    }

    final updatedState = state;
    if (updatedState is CodeTypesSuccess) {
      return updatedState.codes?.where((code) => code.codeTypeModel?.id == addressUseCodeType.id).toList() ?? [];
    }
    return [];
  }
}
